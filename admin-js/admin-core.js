...
const INFO_DICT = {
    ...
    gaps: "<strong>Actionable Gaps:</strong> Clients exposed to new regulations logged in the Radar.",
        ...
        snap.forEach(d => {
            ...
            if (['intake_received','under_review','in_production'].includes(c.status)) {
                ...
                if (startTs) {
                    ...
                    if (hrs > 36) { 
                        slaCrit++;
                        cTbl += `<tr><td>${window.esc(c.baseline?.company || c.name || d.id)}</td><td><span class="badge b-production">${c.status}</span></td><td style="color:#d47a7a">${Math.max(0, 48 - Math.round(hrs))}h left</td></tr>`;
                    }
                }
            }
    ...
    try {
        ...
        if (window.allProspects) {
            window.allProspects.forEach(p => {
                if (p.scannerCompleted && new Date(p.updatedAt || p.addedAt).getTime() > oneWeek) weekForms++;
                if (p.status === 'Converted' && new Date(p.updatedAt || p.addedAt).getTime() > oneWeek) weekDeals++;
