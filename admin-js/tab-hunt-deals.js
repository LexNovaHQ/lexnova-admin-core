...
const HuntCore = {
    ...
    // 1. Initialize Real-Time Database Connection
    init: function() {
        ...
        // Mirrors the legacy real-time listener but isolates it from UI errors
...
const HuntExport = {
    ...
    copySpear: async function(pid) {
        const p = HuntCore.getProspectById(pid);
        if (!p) {
    ...
    // 2. The Strict Verbatim Formatter
...
window.loadOutreach = function() {
    ...
    if (typeof HuntCore !== 'undefined') {
        // If data is already loaded from a previous tab visit, force a re-render
        if (HuntCore.state.isLoaded && typeof HuntUI !== 'undefined') {
            HuntUI.renderMainDash();
        } else {
            // Otherwise, initialize Firebase and let the snapshot trigger the render
            HuntCore.init(); 
        }
    }
};
