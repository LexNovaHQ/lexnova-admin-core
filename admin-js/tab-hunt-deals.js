...
const HuntCore = {
    ...
    deleteProspect: async function(docId) {
        try { await window.db.collection('prospects').doc(docId).delete(); return true; } 
        catch (error) { return false; }
    },

    getProspectById: function(id) {
        return this.state.prospects.find(p => p.id === id || p.prospectId === id);
    }
};
...
const HuntUI = {
 ...
 // Root div in your admin panel
    ...
    applyFilters: function(arr) {
        ...
        const statusFilter = document.getElementById('hunt-filter-status')?.value || "ALL";

        return arr.filter(p => {
            const matchSearch = !searchTerm || 
                (p.company || "").toLowerCase().includes(searchTerm) || 
                (p.founderName || "").toLowerCase().includes(searchTerm) ||
                (p.prospectId || "").toLowerCase().includes(searchTerm);
            
            const matchBatch = (batchFilter === "ALL") || (p.batchNumber === batchFilter);
    ...
    attachDashListeners: function() {
        ['hunt-search', 'hunt-filter-batch', 'hunt-filter-status'].forEach(id => {
            ...
            if (el) {
                el.oninput = () => this.renderMainDash();
            }
        });
    },
    // ════════════════════════════════════════════════════════════════════════
